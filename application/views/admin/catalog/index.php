	...
	?>
		<tr class="cattree-tr <?=!empty($item['childs']) ? 'not_empty' : null; ?>" data-parent="<?=$item['idParent'];?>" data-id="<?=$item['idItem'];?>" style="<?=$item['idParent'] != 0 ? 'display: none;' : null;?>">
			...
			<td class="text-right" nowrap>
				<?=$item['home'] == 1 ? '<span class="tooltips mr10" data-original-title="Отображается на главной">'.fa('home fa-fw text-gray').'</span>' : '';?>
				<?=$item['tmenu'] == 1 ? '<span class="tooltips mr10" data-original-title="Отображается в меню">'.fa('bars fa-fw text-gray').'</span>' : '';?>
