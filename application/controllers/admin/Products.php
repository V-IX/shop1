...
class Products extends ADMIN_Controller {
	...
	{
		$this->db->select('idItem, title, alias, brief, text, mTitle, mKeywords, mDescription');
		$items = $this->query->items('shop_products', false, false, false, array('idItem >' => 44));
		...
		foreach($items as $item)
		{
			...
			foreach($item as $k => $v)
			{
				if($k == 'alias') $v = str_replace('eslajt', 'lihter', $v);
				else $v = str_replace('Эслайт', 'Лихтер', $v);
				$ins[$k] = $v;
			}
			//$this->db->update('shop_products', $ins, array('idItem' => $item['idItem']));
			//$return[$item['idItem']] = $ins;
		}
