	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data.inTotalQuantity == 0) {
				$('.cart-form-wrap').html('Нет добавленных товаров');
			}
...
function removeFromCart(rowid) {
	$.ajax({
		...
		success : function(data) {
			...
			if (data.inTotalQuantity == 0) {
				$('.cart > .wrapper').html('<div class="catalog-empty">В корзине нет товаров. <a href="' + base_url + 'catalog">Перейти в каталог</a>.</div>');
			}
			else if (data.deleted) {
				$("[data-cart-rowid=" + rowid +"]").remove();
			}
...
function updateInTotal(data) {
	...
	if(data.inTotalQuantity == 0) $('.minicart').addClass('_empty');
	else $('.minicart').removeClass('_empty');
